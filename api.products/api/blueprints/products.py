...
def get_all_products():
    ...
    try:
        if status_filter is None:
            ...
            products = Product.select().where(Product.ProductStatus == status_filter).dicts()
        product_serialized = product_schema.dump(products)
    except Exception as err:
        return { 'data': [], 'message': str(err) }, 500
    return { 'data': product_serialized, 'message': '' }, 200
